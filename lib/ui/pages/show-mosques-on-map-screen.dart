...
class ShowMosquesOnMapScreen extends StatefulWidget {
  final masjids;

  ShowMosquesOnMapScreen({this.masjids});

  @override
  _ShowMosquesOnMapScreenState createState() => _ShowMosquesOnMapScreenState();
}

class _ShowMosquesOnMapScreenState extends State<ShowMosquesOnMapScreen> {
...
//    _animateToMosqueLocation();
    ...
    await _setUpMarkersList();
    setState(() {});
  }

  _setUpMarkersList() {
    var lat, long;
    for (Masjid masjid in widget.masjids) {
  ...
  Future<void> _setCustomMapPins() async {
    ...
    jamiaMasjidPinIcon = await BitmapDescriptor.fromAssetImage(
        ImageConfiguration(), 'assets/static_assets/jamia-masjid-pin.png');
    setState(() {});
  }
